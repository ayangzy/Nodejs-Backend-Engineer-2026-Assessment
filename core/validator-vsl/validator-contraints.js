...
function isslug(value, arg, isNot, prop) {
  ...
  const isSatisfied = !!result;
  if (Array.isArray(value)) {
    return processArray(isslug, value, [arg, isNot], prop);
  }
  return {
    evaluatedValue: value,
    isSatisfied,
    errorMessage: `Slug can only contain letters, numbers, hyphens and underscores`,
  };
}

function ishttpurl(value, arg, isNot, prop) {
  let result = value.startsWith('http://') || value.startsWith('https://');
  if (isNot) result = !result;
  if (Array.isArray(value)) return processArray(ishttpurl, value, [arg, isNot], prop);
  return {
    evaluatedValue: value,
    isSatisfied: !!result,
    errorMessage: `Passed ${prop} must start with http:// or https://`,
  };
}

function notempty(value, arg, isNot, prop) {
  let result = true;
  ...
  return {
    evaluatedValue: value,
    isSatisfied: result,
    errorMessage: `Passed ${prop} cannot be empty`,
  };
}

function isinteger(value, arg, isNot, prop) {
  let result = Number.isInteger(Number(value));
  if (isNot) result = !result;
  if (Array.isArray(value)) return processArray(isinteger, value, [arg, isNot], prop);
  return {
    evaluatedValue: value,
    isSatisfied: result,
    errorMessage: `Passed ${prop} must be an integer without decimals`,
  };
}
